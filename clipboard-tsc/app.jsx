...
const Icon = {
  ...
  Star: (p) => <svg {...p} width="16" height="16" viewBox="0 0 24 24" fill={p.filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
...
function App() {
  ...
  const [dragging, setDragging] = useState(null); // {sectionIdx, itemIdx}
  ...
  const moveItem = (from, to) => {
    if (!from || !to) return;
    if (from.sectionIdx === to.sectionIdx && from.itemIdx === to.itemIdx) return;

    setData(d => {
      const nd = d.map(s => ({ ...s, items: [...s.items] }));
      const fromSection = nd[from.sectionIdx];
      const toSection = nd[to.sectionIdx];
      if (!fromSection || !toSection) return d;

      const [moved] = fromSection.items.splice(from.itemIdx, 1);
      if (!moved) return d;

      if (from.sectionIdx === to.sectionIdx) {
        const insertAt = from.itemIdx < to.itemIdx ? to.itemIdx - 1 : to.itemIdx;
        toSection.items.splice(insertAt, 0, moved);
      } else {
        toSection.items.splice(to.itemIdx, 0, moved);
      }
      return nd;
    });
  };
              ...
              {section.items.map((item, iIdx) => {
                ...
                return (
                  <ClipCard
                    ...
                    dragging={dragging}
                    onDragStart={() => setDragging({ sectionIdx: realSectionIdx, itemIdx: realIdx })}
                    onDragEnter={(e) => {
                      e.preventDefault();
                      if (!dragging) return;
                      moveItem(dragging, { sectionIdx: realSectionIdx, itemIdx: realIdx });
                      setDragging({
...
// ============ Clip card ============
function ClipCard({ item, isFav, isCopied, onCopy, onFav, onEdit, showShapes, dragging, onDragStart, onDragEnter, onDragEnd }) {
  ...
  return (
    <div
      className={`clip-card card ${sizeClass} ${colorClass} ${isCopied ? "copied" : ""} ${dragging && dragging.sectionIdx !== undefined ? "drag-active" : ""}`}
      onClick={onCopy}
      draggable
    ...
    >
      <div className="card-tl">
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button className={`icon-btn ${arrowColor}`} onClick={(e) => { e.stopPropagation(); onFav(); }} title="Favorito" type="button">
            <Icon.Star filled={isFav} />
          </button>
          <button className={`icon-btn ${arrowColor}`} onClick={(e) => { e.stopPropagation(); onEdit(); }} title="Editar" type="button">
            <Icon.Edit />
          </button>
        </div>
      </div>
      ...
      <div className="card-bl">
        ...
        <button className={`icon-btn ${arrowColor}`} onClick={(e) => { e.stopPropagation(); onCopy(); }} title="Copiar" type="button">
